...
class AdjuntosRequest(BaseModel):
    ...
    def validate_fotos(cls, v):
        ...
        for foto in v:
            ...
            if not re.match(r'^[A-Za-z0-9+/]*={0,2}$', foto.replace('data:image/', '').split(',')[-1]):
...
class InversionRequest(BaseModel):
    ...
    fecha_creacion: str = Field(default_factory=lambda: date.today().isoformat(), description="Fecha de creación del reporte")
